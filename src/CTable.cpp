...
CCell CRow::getCell(size_t pos) const { return m_row.at(pos); }
...
void CRow::print(size_t rowNum, size_t x, const CTable &table) {
  ...
  cout << setw(10) << rowNum
       << "|"; // print number of the row in front of the row
  double res = 0;
...
void CTable::insert(size_t y, size_t x, const string &val) {
  // print out the cell and value that will be inserted
    cout << endl;
    cout << "Bunka: [" << y << ";" << x << "]" << endl;
    cout << val << endl;
  ...
  // transform coordinates to print inserted cell in center of the table or
  // print 0;0 top left corner table
  int y1, x1;
  if (y >= 5) {
    y1 = y - 5;
  } else {
...
void CTable::print(size_t y, size_t x) {
  // if the area that we want to print is bigger than our tabel, we must resize
  // it
  ...
  cout << endl;
  cout << setw(10) << " "
       << "|"; // prints empty cell in left top corner
