 ...
 *
   \brief This is the file where all mathematical functions are defined.
         All mathematical functions and all functions related to evaluating
 mathematical expression in cells
...
public:
  virtual const char *what() const noexcept { return "Brackets missmatch"; }
};

/**
   \class OperatorError
   \brief Class is used to be throw as an exception when operator error happens.
   Error could be for example operator at the end of expressin or binary
   operator with only one operand etc.

 */
class OperatorError : public std::exception {
public:
  virtual const char *what() const noexcept { return "OperatorError"; }
};
...
findAncestors(std::set<std::pair<int, int>> &ancestors,
              std::set<std::pair<int, int>> parents, const CTable &table);
...
std::vector<std::shared_ptr<CToken>>
infixToRPN(const std::vector<std::string> &vec);
