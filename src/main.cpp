 ...
 * \date April 9, 2017, 10:55 AM
   ...
   Simple Spreadsheet editor created by Stepan Severa (severste) for subject
 Programming and algorithmic 2 in summer semester 2016/2017
   \subsection desc Basic decripiton
   It is a simple Spreadsheet editor as you can see in title. You can insert
 text or mathematical expressions into the cells.\n
   Mathematical expression must start with =. In math. expressions you can use
 mathematical operators: +, -, *, /, ^(power) or
   some basic functions: sin(x): sinus, cos(x): cosinus, tan(x): tangens,
 cotg(x): cotangens, abs(x): absolute value, exp(x): exponencial function,
   log(x): decimal logarithm, ln(x): natural logarithm, sqrt(x): square root.
   To evaluate mathematical expressions I use shunting yard alrgorithm
 https://en.wikipedia.org/wiki/Shunting-yard_algorithm
   \subsection controls Controls:
   'p' (print) Print table. Table is 11x11 large. Inserted coords are of the top
 left corner.\n
                Maximum coords are: 499 990\n
   'i' (insert) Insertion into the table. Input coords and then inserted
 value.\n
                Maximum coords are:  500 000\n
                To insert mathematical expression the first char must be =\n
                To insert link to another cell use [y;x], where y is row number
 and x is column number.\n
...
/**
  \def END
   \brief END is constant that returns inputHandler if user whishes to end
  program
*/
#define END 0
/**
\def PRINT
 \brief PRINT is constant that returns inputHandler if user whishes to print
table
*/
#define PRINT 1
/**
\def INSERT
\breif INSERT is constant that returns inputHandler if user whishes to insert
into cell
*/
#define INSERT 2
/**
\def PRINTCELL
\brief PRINTCELL is constant that returns inputHandler if user whishes to print
whole cell
*/
#define PRINTCELL 3
/**
\def HELP
\brief HELP is constant that returns inputHandler if user whishes to print the
help
*/
#define HELP 4
/**
...
/**
  \class InvalidCoords
  \brief Class is used to be throw as an exception when user inputs invalid
  coords for example negative coords or NaN coords
*/
class InvalidCoords : public exception {
public:
  virtual const char *what() const noexcept { return "Neplatne souradnice"; }
};
...
/**
  ...
  @return constant defined in this file, for example if user wants to print
  table it return PRINT
*/
int inputHandler() {
  char x;
  cin >> x;
  ...
  case 'q':
    ...
    break;
  default:
    cout << "Zadejte platne hodnoty. Stisknete h pro napovedu" << endl;
    return -1;
  }
...
/**
  ...
  CTable cannot be const because it also resizes the table if the printable area
  is out of table
...
void printTable(CTable &table) {
  ...
  if (!error) {
    // prints also resizes the table if the printable area is out of range so
...
/**
  ...
  If coords are bigger than our table it resizes itself
  Table cannot be const because it resizes it and evaluetes the parent cells.
...
void printCell(CTable &table) {
  ...
  if (!error) {
    cout << "Bunka(" << y << "," << x << "): ";
    /* print content evaluates cell and if parent is not inicialised, it
      inicialise it so
      table cannot be const */
...
/**
  ...
  @return int END(defined) if the program should end and START(defined) if
