 ...
 * \file CToken.h
 * \author severste
* \brief This is the file where CToken, CNumber, COperator and CFunction classes
* are defined
 */
...
/**
  ...
  It is used also during transformation vector<string> infix notation intto RPN
  and evaluate the RPN
...
public:
  ...
  */
  ...
  @return double, if token is a CNumber returns its value, if token is not a
  number returns 0
  */
...
class CFunction : public CToken {
  ...
  @param string function - represents the string representation of function for
  example: sin
  */
public:
  ...
  virtual bool isNumber() const;
  /**
                Method performOperation pops value out of a stack and performs
     operation according to the function used
                and result push back to a stack
                @param stack<double> stack - is a stack where are double values
     of CNumber from evaluating RPN
        */
  virtual void performOperation(std::stack<double> &stack);
  virtual double getVal() const;
...
public:
  ...
  @param string o - represents the string representation of operator for
  example: +
  */
  COperator(std::string o);
  virtual bool isNumber() const;
  /**
    Method performOperation pops 2 values out of a stack and performs operation
    between them according to the operator used
    and result push back to a stack
    @param stack<double> stack - is a stack where are double values of CNumber
    from evaluating RPN
  */
  virtual void performOperation(std::stack<double> &stack);
  virtual double getVal() const;
